...
Prose throughout this repository, in the specs, the plans, the source comments
and the tests, refers to this document as `CLAUDE.md`, which is what it was
called until it moved. Those references are historical records and are correct
about the content; read them as naming this file.
...
**So: build a test only when it asserts something measurable, and hand a visual
change to the user instead.** This is the rule the whole section above serves,
stated by the user on 2026-08-26, and it is the cheaper half of it: the traps
listed here are all ways a test over a visual property reports success forever,
and the reliable way to avoid them is not to write that test.

Test the things with a right answer: the geometry (`CardLayout`), the generated
string, the tag write, the parse, the query `SearchTerm` builds. Do not test
"does it look right". When unsure which side a property falls on, ask whether a
mutation could make the test fail; if the answer is no, the test is not worth
building, whatever it appears to cover.

**A green suite is not evidence a UI design is right, and the precedent cost a
whole feature.** Item 20's message rows were finished, tested and green when the
user rejected them on sight: "I don't think the table view fits our use." The
card list replaced them and shipped. So for any change the user will SEE, hand
the build over and let them look before treating it as done, whatever the suite
says. Say what to look for; do not launch the application yourself, per the rule
at the top of this file. Branch substantial UI work so master stays shippable,
since the user runs it daily while it is being built.
...
**Then write the same table to the user's vault**, at
`~/Documents/Obsidian/note/qtmaildir - backlog.md`. It exists so the user can
follow the backlog from their own notes rather than by reading a five-thousand
line file in the repo, and it is a SNAPSHOT: the repo's status table stays the
source of truth, and this note is regenerated from it. So rewrite it whole on
every reconcile rather than editing rows in place, which is what keeps a closed
item from lingering there after it has left the backlog.

Three things about it are conventions of the vault, not of this repo, and a
file written without them reads as foreign there: no frontmatter, links to the
sibling notes as wikilinks (`[[notes on qtmaildir]]`, `[[qtmaildir - Done]]`,
`[[qtmaildir shortcuts and menu structure]]`), and the same defect/enhancement
split the terminal table uses. Keep the header line stating the date and saying
the numbers are the backlog's own, since they do not match the numbering in the
user's notes and never will.

Nothing about the personal-data rule is relaxed by writing outside the repo:
the note is in a vault the user syncs, so it carries item numbers and
descriptions, never mail content.
