...
| 20 | Thread view does not match the user's mental model | presentation | L | **built 2026-08-08**, branch `item-20-message-rows`; see 53 |
...
**Built 2026-08-08 across ten tasks**, all four decisions implemented: message
...
`AuthorsColumn` and `SubjectColumn` for message rows at
`src/threadlistmodel.cpp:275-283`, mirroring the thread-row branch at
`:428-431`. One model, one column grid, both row kinds. So every reply lands on
the same rigid column boundaries as the threads around it, and the eye reads
columns before it reads indentation or tint.
...
- Reply rows keep the same row height as thread rows, since
  ...
  (`src/mainwindow.cpp`). A conversation view would want tighter replies.
